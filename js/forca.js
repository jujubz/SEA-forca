...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function setRandomPosition(img) {
        ...
        const randomX = Math.random() * (windowWidth - img.width);
        const randomY = Math.random() * (windowHeight - img.height);
    ...
    function animateFloating(img) {
        ...
        const randomSpeed = 1 + Math.random() * 2; // Velocidade entre 1 e 3 pixels por segundo

        const interval = setInterval(function() {
            const currentLeft = parseFloat(img.style.left);
            const currentTop = parseFloat(img.style.top);

            const dx = randomSpeed * Math.cos(randomAngle * Math.PI / 180);
            const dy = randomSpeed * Math.sin(randomAngle * Math.PI / 180);
            ...
            // Verifica se a imagem saiu da tela e reposiciona
            if (newLeft < -img.width || newLeft > window.innerWidth || 
                newTop < -img.height || newTop > window.innerHeight) {
                setRandomPosition(img);
            }
        }, 1000 / 60); // 60 FPS
    }
